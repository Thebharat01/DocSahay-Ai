export default function RefundCancellation() {
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-10">
      <div className="mx-auto max-w-4xl rounded-2xl bg-white p-6 shadow-sm md:p-10">

        <h1 className="mb-3 text-3xl font-bold text-gray-900">
          Refund & Cancellation Policy
        </h1>

        <p className="mb-8 text-sm text-gray-500">
          Last Updated: September 1, 2026
        </p>

        <div className="space-y-7 text-gray-700 leading-7">

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              1. Introduction
            </h2>
            <p>
              At DocSahay-AI, we aim to provide reliable and useful
              AI-powered document analysis services. This Refund &
              Cancellation Policy explains the conditions under which
              cancellations and refunds may be requested.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              2. Digital Services
            </h2>
            <p>
              DocSahay-AI primarily provides digital and AI-powered services.
              Because these services are delivered electronically, a refund
              may not be available once the purchased service has been
              successfully used or the requested AI processing has been
              completed.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              3. Cancellation Before Service Use
            </h2>
            <p>
              If you have made a payment but have not yet used or initiated
              the purchased service, you may contact us as soon as possible
              to request cancellation.
            </p>
            <p className="mt-3">
              Cancellation requests will be reviewed based on whether the
              service has already been initiated or delivered.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              4. Refund Eligibility
            </h2>

            <p>
              A refund may be considered in situations such as:
            </p>

            <ul className="mt-3 list-disc space-y-2 pl-6">
              <li>
                Payment was successfully deducted but the purchased service
                was not made available due to a technical issue on our side.
              </li>
              <li>
                A duplicate payment was accidentally made for the same
                transaction.
              </li>
              <li>
                A technical problem prevented delivery of the purchased
                service and the issue could not reasonably be resolved.
              </li>
              <li>
                A refund is required under applicable law.
              </li>
            </ul>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              5. Non-Refundable Situations
            </h2>

            <p>
              Refunds may generally not be provided in the following cases:
            </p>

            <ul className="mt-3 list-disc space-y-2 pl-6">
              <li>
                The user has already successfully used the purchased service.
              </li>
              <li>
                AI processing has already been completed and the result has
                been delivered.
              </li>
              <li>
                The user does not like or agree with an AI-generated result,
                where the service itself was successfully provided.
              </li>
              <li>
                Incorrect or incomplete information was provided by the user.
              </li>
              <li>
                The user uploaded an unsupported, corrupted, or unreadable
                document.
              </li>
              <li>
                The user changed their mind after successfully using the
                service.
              </li>
            </ul>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              6. Duplicate Payments
            </h2>

            <p>
              If you believe that you have accidentally made a duplicate
              payment for the same service, please contact us with the
              transaction details. After verification, the duplicate amount
              may be refunded through the applicable payment method.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              7. Failed Transactions
            </h2>

            <p>
              If your bank account or payment method has been debited but
              the transaction was not successfully confirmed by our system,
              please contact us with the transaction details.
            </p>

            <p className="mt-3">
              Once the payment status is verified, any applicable reversal
              or refund will be processed through the payment gateway or
              payment method used for the transaction.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              8. Refund Processing Time
            </h2>

            <p>
              Once a refund is approved, we will initiate the refund through
              the applicable payment gateway or payment method.
            </p>

            <p className="mt-3">
              The time required for the refunded amount to appear in your
              bank account or payment account may vary depending on your
              bank, card issuer, UPI provider, or payment gateway.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              9. How to Request a Refund
            </h2>

            <p>
              To request a refund or report a payment issue, contact us at:
            </p>

            <div className="mt-4 rounded-xl bg-gray-100 p-4">
              <p>
                <strong>Email:</strong>{" "}
                <a
                  href="mailto:info.thebharatfeed@gmail.com"
                  className="text-blue-600 hover:underline"
                >
                  info.thebharatfeed@gmail.com
                </a>
              </p>

              <p className="mt-2">
                Please include your payment/order details, transaction ID,
                date of payment, and a brief explanation of the issue.
              </p>
            </div>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              10. Refund Verification
            </h2>

            <p>
              We may verify the transaction and service usage before
              approving a refund. Additional information may be requested
              where necessary to investigate a payment issue.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              11. Changes to This Policy
            </h2>

            <p>
              DocSahay-AI may update this Refund & Cancellation Policy from
              time to time. Any updated version will be published on this
              page with a revised “Last Updated” date.
            </p>
          </section>

          <section>
            <h2 className="mb-2 text-xl font-semibold text-gray-900">
              12. Contact Us
            </h2>

            <p>
              If you have questions regarding refunds, cancellations,
              payments, or transactions, please contact us using the
              information below.
            </p>

            <div className="mt-4 rounded-xl bg-gray-100 p-4">
              <p>
                <strong>Website:</strong> DocSahay-AI
              </p>

              <p className="mt-2">
                <strong>Email:</strong>{" "}
                <a
                  href="mailto:info.thebharatfeed@gmail.com"
                  className="text-blue-600 hover:underline"
                >
                  info.thebharatfeed@gmail.com
                </a>
              </p>
            </div>
          </section>

          <div className="mt-8 rounded-xl bg-gray-100 p-4 text-sm">
            By making a purchase on DocSahay-AI, you acknowledge that you
            have read and understood this Refund & Cancellation Policy.
          </div>

        </div>
      </div>
    </main>
  );
}
